...
function wrapper(plugin_info) {
	...
	window.plugin.linksFields = {
				...
				if (validity[i][0] != "exists") {
					if (old_point) {
						...
						if (link.type == 'polyline') {
							poly = L.polyline([old_point, link.latLngs[0]], window.plugin.linksFields.PATH_STYLE);
						}
						if (link.type == 'marker') {
							poly = L.polyline([old_point, link.latLng], window.plugin.linksFields.PATH_STYLE);
						}
						poly.addTo(window.plugin.linksFields.pathLayer);
						poly.addTo(window.plugin.linksFields.blockLayer);
					}
					if (link.type == 'polyline') {
						old_point = link.latLngs[0];
					}
					if (link.type == 'marker') {
						old_point = link.latLng;
					}
				}
			}
		},
		...
		// ============================================================================
		// OLD PHASE 1 OPTIMIZER FUNCTIONS REMOVED
		// These functions were part of the original Phase 1 optimizer and have been
		// replaced by the semi-automated optimizer (semiAutomatedOptimize and related functions).
		// Removed functions: optimizeLinkDirections, evaluateLinkDirection, reverseLink,
		// isPortalVisited, getBlockingFields, insertMarkers, greedyFieldOrder,
		// calculateLinkScore, identifyWastedFields, fieldContains, calculateFieldSize
		// ============================================================================

		// ============================================================================
		// SEMI-AUTOMATED OPTIMIZER
		// ============================================================================

		/**
		 * Semi-automated optimization with iterative stabilization
		 * Iterates until plan converges (no blockers, no wasted fields, grouped portals)
		 */
		semiAutomatedOptimize: function () {
			console.log('Semi-Automated Optimization: Starting...');

			let optimized = window.plugin.linksFields.getDrawTools();
			let previousPlan = null;
			let iterations = 0;
			const maxIterations = 10;

			// Iterate until plan stabilizes
			while (!this.plansEqual(optimized, previousPlan) && iterations < maxIterations) {
				previousPlan = this.copyPlan(optimized);
				iterations++;

				console.log(`  Iteration ${iterations}:`);

				// Step 1: Remove existing markers (clean slate)
				console.log('    - Removing markers...');
				optimized = this.removeMarkers(optimized);

				// Step 2: Fix wasted fields
				console.log('    - Fixing wasted fields...');
				optimized = this.fixWastedFields(optimized);

				// Step 3: Fix blocked links
				console.log('    - Fixing blocked links...');
				optimized = this.fixBlockedLinks(optimized);

				// Step 4: Insert markers for visit order (before grouping!)
				console.log('    - Inserting markers...');
				optimized = this.insertMarkersForVisitOrder(optimized);

				// Step 5: Group by portal (after markers are in place)
				console.log('    - Grouping by portal...');
				optimized = this.groupByOrigin(optimized);
			}

			if (iterations >= maxIterations) {
				console.warn('  Warning: Did not converge after', maxIterations, 'iterations');
			} else {
				console.log(`  Converged after ${iterations} iteration(s)`);
			}

			console.log('Semi-Automated Optimization: Complete!');

			// Update draw tools
			window.plugin.drawTools.drawnItems.clearLayers();
			window.plugin.drawTools.import(optimized);
		...
		/**
		 * Remove all markers from the plan
		 */
		removeMarkers: function (linkList) {
			return linkList.filter(item => item.type !== 'marker');
		},

		/**
		 * Insert markers to define portal visit order
		 * Inserts markers for destinations that haven't been visited yet (for key farming)
		 */
		insertMarkersForVisitOrder: function (linkList) {
			const result = [];
			const visited = new Set();

			for (let item of linkList) {
				if (item.type === 'polyline') {
					const originKey = this.pointToKey(item.latLngs[0]);
					const destKey = this.pointToKey(item.latLngs[1]);

					// If destination hasn't been visited, insert marker for it
					if (!visited.has(destKey)) {
						result.push({
							type: 'marker',
							latLng: item.latLngs[1],
							color: item.color
						});
						visited.add(destKey);
					}
		...
		/**
		 * Deep copy a plan for comparison
		 */
		...
		/**
		 * Check if two plans are equal (for convergence detection)
		 */
		plansEqual: function (plan1, plan2) {
			...
			for (let i = 0; i < plan1.length; i++) {
				...
				if (item1.type === 'polyline') {
					if (!this.point_equal(item1.latLngs[0], item2.latLngs[0])) return false;
					if (!this.point_equal(item1.latLngs[1], item2.latLngs[1])) return false;
				} else if (item1.type === 'marker') {
					if (!this.point_equal(item1.latLng, item2.latLng)) return false;
				}
			}
		...
		/**
		 * Fix blocked links by reordering to avoid origins inside fields
		 * A link is blocked if its origin is inside a field created by earlier links
		 */
		fixBlockedLinks: function (linkList) {
			const existingLinks = linkList.filter(l => l.type === 'polyline' && window.plugin.linksFields.linkInPlay(l));
			const plannedLinks = linkList.filter(l => l.type === 'polyline' && !window.plugin.linksFields.linkInPlay(l));
			const markers = linkList.filter(l => l.type === 'marker');

			const ordered = [...existingLinks];
			const remaining = [...plannedLinks];
			...
			while (remaining.length > 0 && maxIterations > 0) {
				...
				// Find first link that isn't blocked
				for (let i = 0; i < remaining.length; i++) {
					const link = remaining[i];
					const isBlocked = this.isLinkBlocked(link, ordered);

					if (!isBlocked) {
						// Add this link and remove from remaining
						ordered.push(link);
				...
				// If all remaining links are blocked, take the least-blocked one
				if (!foundUnblocked && remaining.length > 0) {
					let bestIdx = 0;
					let bestScore = Infinity;

					for (let i = 0; i < remaining.length; i++) {
						const score = this.calculateBlockingScore(remaining[i], ordered);
						if (score < bestScore) {
							bestScore = score;
							bestIdx = i;
						}
		...
		/**
		 * Check if a link's origin is blocked by existing fields
		 */
		isLinkBlocked: function (link, currentPlan) {
			const origin = link.latLngs[0];
			const distance = this.haversine_distance(link.latLngs[0], link.latLngs[1]);

			// Short links (<2km) can originate under fields
			if (distance < 2000) {
				return false;
			}

			// Check if origin is inside any field
			const fields = this.fieldify(currentPlan);
			for (let field of fields) {
				if (field.type === 'polygon') {
					if (this.interior_tri(origin, field.latLngs[0], field.latLngs[1], field.latLngs[2], false)) {
						return true;
					}
				}
			}
		...
		/**
		 * Calculate blocking score (lower is better)
		 * Considers: number of blocking fields, field sizes, link distance
		 */
		calculateBlockingScore: function (link, currentPlan) {
			let score = 0;
			const origin = link.latLngs[0];
			const distance = this.haversine_distance(link.latLngs[0], link.latLngs[1]);

			// Short links can go under fields
			if (distance < 2000) {
				return 0;
			}

			// Count blocking fields
			const fields = this.fieldify(currentPlan);
			for (let field of fields) {
				if (field.type === 'polygon') {
					if (this.interior_tri(origin, field.latLngs[0], field.latLngs[1], field.latLngs[2], false)) {
			...
			// Prefer shorter links (easier to fit in)
			score += distance * 0.01;
		...
		/**
		 * Fix wasted fields by reordering links to maximize MU
		 * Uses greedy algorithm to minimize wasted fields
		 */
		fixWastedFields: function (linkList) {
			const existingLinks = linkList.filter(l => l.type === 'polyline' && window.plugin.linksFields.linkInPlay(l));
			const plannedLinks = linkList.filter(l => l.type === 'polyline' && !window.plugin.linksFields.linkInPlay(l));
			const markers = linkList.filter(l => l.type === 'marker');
			...
			while (remaining.length > 0) {
				...
				// Find link that creates fewest wasted fields
				for (let i = 0; i < remaining.length; i++) {
					const testPlan = [...ordered, remaining[i]];
					const score = this.scoreLink(remaining[i], testPlan);
		...
		/**
		 * Helper: Reverse a link (swap origin and destination)
		 */
		reverseLink: function (link) {
			return {
				type: link.type,
				latLngs: [link.latLngs[1], link.latLngs[0]],
				color: link.color
			};
		},

		/**
		 * Group links by origin portal
		 * Preserves topmost portal order - already optimized plans won't change
		 */
		groupByOrigin: function (linkList) {
			const result = [];
			...
			for (let i = 0; i < linkList.length; i++) {
				...
				if (item.type === 'polyline') {
					...
					// Find all other links with same origin
					for (let j = i + 1; j < linkList.length; j++) {
						if (processed.has(j)) continue;

						const otherItem = linkList[j];
						if (otherItem.type === 'polyline') {
							const otherOriginKey = this.pointToKey(otherItem.latLngs[0]);

							if (originKey === otherOriginKey) {
		...
		/**
		 * Swap links where origin is a dependency (was a destination of a previous link)
		 * This breaks cyclic dependencies
		 */
		swapDependencies: function (linkList) {
			const visited = new Set();
			const dependencies = new Set(); // Portals that are destinations
			const result = [];

			for (let item of linkList) {
				if (item.type === 'marker') {
					visited.add(this.pointToKey(item.latLng));
					result.push(item);
					...
					// If origin is a dependency (was a destination before), SWAP
					if (dependencies.has(originKey)) {
						result.push(this.reverseLink(item));
						visited.add(destKey); // New origin after swap
						dependencies.add(originKey); // New destination after swap
					} else {
						result.push(item);
						visited.add(originKey);
						dependencies.add(destKey);
					}
				}
			}
		...
		/**
		 * Optimize link order within each portal group to avoid wasted fields
		 * Reorders links from the same origin to minimize field completion issues
		 */
		optimizeFieldOrder: function (linkList) {
			const result = [];
			let i = 0;
			...
			while (i < linkList.length) {
				...
				// Collect all links with the same origin
				const originKey = this.pointToKey(item.latLngs[0]);
				const sameOriginLinks = [item];
				let j = i + 1;

				while (j < linkList.length && linkList[j].type === 'polyline') {
					const nextOriginKey = this.pointToKey(linkList[j].latLngs[0]);
					if (nextOriginKey === originKey) {
						sameOriginLinks.push(linkList[j]);
						j++;
					} else {
						break;
					}
				...
				// If only one link, no need to optimize
				if (sameOriginLinks.length === 1) {
					result.push(item);
					i++;
					continue;
				}

				// Reorder links to minimize wasted fields
				const ordered = this.orderLinksToMinimizeWaste(sameOriginLinks, result);
				result.push(...ordered);
				i = j;
			}
		...
		/**
		 * Order a group of links from the same origin to minimize wasted fields
		 * Uses greedy algorithm: pick link that creates fewest wasted fields
		 */
		orderLinksToMinimizeWaste: function (links, priorPlan) {
			const ordered = [];
			...
			while (remaining.length > 0) {
				...
				// Try each remaining link and score it
				for (let i = 0; i < remaining.length; i++) {
					const testPlan = [...priorPlan, ...ordered, remaining[i]];
					const score = this.scoreLink(remaining[i], testPlan);
		...
		/**
		 * Score a link based on field completion issues
		 * Lower score is better
		 */
		scoreLink: function (link, currentPlan) {
			let score = 0;

			// Check what fields this link completes
			const completedFields = window.plugin.linksFields.complete_field(currentPlan, link);

			if (completedFields.length > 2) {
				// More than 2 fields = wasted fields
				score += (completedFields.length - 2) * 100;
			} else if (completedFields.length === 2) {
				// Check if both fields are on the same side (wasted)
				const field1 = [link.latLngs[0], link.latLngs[1], completedFields[0]];
				const field2 = [link.latLngs[0], link.latLngs[1], completedFields[1]];

				if (window.plugin.linksFields.field_contains(field1, field2) || window.plugin.linksFields.field_contains(field2, field1)) {
					score += 50; // Same side = wasted
				}
			}
		...
		/**
		 * Insert markers for unvisited destinations
		 * Only inserts markers for portals that don't appear as origins later
		 */
		insertMarkersForDependencies: function (linkList) {
			// Find all portals that appear as link origins
			const originsInPlan = new Set();
			for (let item of linkList) {
				if (item.type === 'polyline') {
					originsInPlan.add(this.pointToKey(item.latLngs[0]));
				}
			}
			...
			for (let item of linkList) {
				if (item.type === 'marker') {
					...
					// Only insert marker if destination is NOT an origin elsewhere
					// This prevents multiple visits to the same portal
					if (!visited.has(destKey) && !originsInPlan.has(destKey)) {
						result.push({
							type: 'marker',
							latLng: item.latLngs[1],
							color: item.color
		...
		/**
		 * Helper: Convert point to string key
		 */
		pointToKey: function (point) {
			return `${point.lat},${point.lng}`;
		},

		/**
		 * Helper: Convert string key to point
		 */
		keyToPoint: function (key) {
		...
		linkOriginEqPortal: function (link, portal) {
			...
			var loLngE6;
			if (link.type == "polyline") {
				loLatE6 = link.latLngs[0].lat * 1000000;
				loLngE6 = link.latLngs[0].lng * 1000000;
			} else if (link.type == "marker") {
				loLatE6 = link.latLng.lat * 1000000;
				loLngE6 = link.latLng.lng * 1000000;
			}
			//console.log("lolatE6: " + loLatE6 + " polatE6: " + portal.latE6 + " loLngE6: " + loLngE6 + " polngE6: " + portal.lngE6);
			return (loLatE6 == portal.latE6 && loLngE6 == portal.lngE6);
		},
		sortPortalToTop: function (link_list, selected_portal) {
			console.log('sortPortalToTop: selected_portal =', selected_portal);
			var sort_list = [];
			var i;
			...
			console.log('sortPortalToTop: sort_list.length =', sort_list.length);
		...
		sortPortalToBottom: function (linkList, selectedPortal) {
			console.log('sortPortalToBottom: selectedPortal =', selectedPortal);
			const sortList = [];
			...
			console.log('sortPortalToBottom: sortList.length =', sortList.length);
		...
		makeHtml: function (link_list) {
			var html;
			html = '<button type="button" onclick="window.plugin.linksFields.semiAutomatedOptimize()">Optimize Plan</button>';
		...
		sortToTop: function () {
			...
			if (window.selectedPortal == null) {
				alert('Please select a portal first by clicking on it in the map.');
				return;
			}
